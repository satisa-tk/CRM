SELECT YEAR_MONTH,
    COUNT ( CASE WHEN CUSTOMER_TYPE = "NEW CUSTOMER" THEN 1
    END) AS NEW_CUSTOMER,
    
    COUNT(CASE WHEN CUSTOMER_TYPE = "REPEAT CUSTOMER" THEN 1
    END) AS REPEAT_CUSTOMER,
 
    COUNT(CASE WHEN CUSTOMER_TYPE = "REACTIVED CUSTOMER" THEN 1
    END) AS REACTIVATED_CUSTOMER, -(LAG(COUNT(YEAR_MONTH), 1)
...
FROM
    ...
    CASE WHEN PREVIOUS_MONTH IS NULL THEN "NEW CUSTOMER"
...
FROM
    (SELECT CUST_CODE, YEAR_MONTH,
    CAST(SAFE.SUBSTR(CAST(YEAR_MONTH AS STRING), 1, 4) AS NUMERIC) AS 
    CAST_YEAR,
    CAST(SAFE.SUBSTR(CAST(YEAR_MONTH AS STRING), 5, 2) AS NUMERIC) AS
    CAST_MONTH,
...
FROM
    (SELECT CUST_CODE, SUBSTRING(CAST(SHOP_DATE AS STRING), 1, 6) AS YEAR_MONTH
    FROM `crm-project-317013.Supermarket.Supermarket`
